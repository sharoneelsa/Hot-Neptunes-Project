...
! ***********************************************************************
      ...
      subroutine extras_controls(id, ierr)
         ...
         write(*,*) ">>> extras_controls: other_surface_PT hook assigned"
      ...
      subroutine core_heating(id, ierr)
         ...
         real(dp) :: chi, age_yr, ln2, heat_per_kg, M_core_kg
         real(dp) :: L_reinflate, reinflate_on, lum_factor
         integer :: ival=300
         ierr = 0
         call star_ptr(id, s, ierr)
         if (ierr /= 0) return
         ...
         M_core = s% x_ctrl(2)
         chi = s% x_ctrl(3)
         reinflate_on = s% x_ctrl(7)   ! 1.0 = on, 0.0 = off
         lum_factor = s% x_ctrl(8)   ! 3.0 for high-mass planets

         age_yr = s%star_age
         ln2 = log(2.0d0)

         heat_per_kg = &
        30.8d-9 * 9.46d-5 * exp(-ln2*age_yr/4.47d9) + & ! U-238
        0.22d-9 * 5.69d-4 * exp(-ln2*age_yr/7.04d8) + & ! U-235
        124.0d-9 * 2.64d-5 * exp(-ln2*age_yr/1.40d10) + & ! Th-232
        36.9d-9 * 2.92d-5 * exp(-ln2*age_yr/1.25d9) ! K-40

         M_core_kg = M_core / 1000.0d0
         L_radio = chi * M_core_kg * heat_per_kg * 1.0d7
         ! Artificial reinflation luminosity
         ! = lum_factor × current surface luminosity
         if (reinflate_on > 0.5d0) then
            L_reinflate = lum_factor * s%L(1)
         else
            L_reinflate = 0d0
         end if
         ...
         s% extra_heat(s%nz) = -cv_core*M_core*dTdt_core/s%dm(s%nz) + L_radio/s%dm(s%nz) + L_reinflate/s%dm(s%nz)
         !ierr = -1
      ...
      subroutine irradiated_atm(id, &
         ...
         type(auto_diff_real_4var_order1) :: L, lnR, lnM, lnkap
         type(auto_diff_real_4var_order1) :: M, R, kap, g
         type(auto_diff_real_4var_order1) :: Tint4, Tirr4, T4, lnT, lnP
         real(dp) :: sqrt3, tau, f_val, cg_val
         ierr = 0
         write(*,*) ">>> irradiated_atm: hook entered" 
         call star_ptr(id, s, ierr)
         if (ierr /= 0) return
         ...
         T4 = 0.75d0*Tint4*(2d0/3d0 + tau) + 0.75d0*Tirr4*f_val*(2d0/3d0 + 1d0/(cg_val*sqrt3) + (cg_val/sqrt3 - 1d0/(sqrt3*cg_val))*exp(-cg_val*tau*sqrt3))
         if (T4%val <= 0d0) then
            write(*,*) "ERROR irradiated_atm: T4 <= 0, T4 = ", T4%val
            write(*,*) "  Tint4 = ", Tint4%val, "  Tirr4 = ", Tirr4%val
            write(*,*) "  f, tau, gamma = ", f_val, tau, cg_val
            ierr = -1
            return
         end if
         lnT = 0.25d0*log(T4)
         ...
         write(*,*) "  T_surf [K]       = ", exp(lnT%val)
         write(*,*) "  P_surf [dyn/cm2] = ", exp(lnP%val)
         write(*,*) "  tau, f, gamma    = ", tau, f_val, cg_val
